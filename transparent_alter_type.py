        ...
        res = self.execute('''
            select tn.table_name as name,
             ...
             cross join lateral (select t.oid::regclass::text as table_name) tn
              left join lateral (select format('comment on table %%s__tat_new is %%L;', tn.table_name, d.description) as comment
                                   from pg_description d
                                  where d.objoid = t.oid and
                                        d.objsubid = 0 and
                                        d.classoid = 'pg_class'::regclass) d
                     on true
             cross join lateral (select coalesce(array_agg(replace(replace(
                                          pg_get_indexdef(i.indexrelid),
                                            ' ON ', '__tat_new ON '),
                                            ' USING ', '__tat_new USING ') order by cardinality(i.indkey) desc), '{}') as create_indexes,
                                        coalesce(array_agg(format(
                                            'alter index %%s.%%s__tat_new rename to %%s;',
                                            icn.nspname, ic.relname, ic.relname)), '{}') as rename_indexes
                                   from pg_index i
             ...
             left join lateral (select uni.contype,
                                  ...
                                  where uni.conrelid = t.oid and
                                        ...
                                        a.attrelid = t.oid
                                  group by 1
                                 having cardinality(array_agg(a.attname order by a.attnum) filter (where a.attnotnull)) = count(*) --only all not null columns
                                  order by count
             ...
             cross join lateral (select coalesce(array_agg(format(
                                            ...
                                            uni.conname,
                                            case when uni.condeferrable then 'deferrable' else '' end,
                                            case when uni.condeferred then 'initially deferred' else '' end)),
                                            '{}') as create_constraints
                                   from pg_constraint uni
                                  where uni.conrelid = t.oid and
                                        uni.contype in ('p', 'u')) uni
             cross join lateral (select coalesce(array_agg(format(
                                            'alter table %%s__tat_new add constraint %%s %%s;',
                                            tn.table_name,
                                            chk.conname,
             ...
             cross join lateral (select coalesce(array_agg(format(
                                        ...
                                        coalesce(array_agg(format(
                                            ...
                                            fk.conrelid::regclass::text,
                                            fk.conname)), '{}') as validate_constraints,
                                        coalesce(array_agg(format(
                                            'alter table %%s drop constraint %%s;',
                                            fk.conrelid::regclass::text,
                                            fk.conname)) filter (where fk.conrelid <> t.oid), '{}') as drop_constraints
                                  ...
                                  where (fk.conrelid = t.oid
                                        ...
                                        fk.contype = 'f') fk
             cross join lateral (select coalesce(array_agg(format(
                                            'grant %%s on table %%s__tat_new to %%s;',
                                            p.privileges,
                                            tn.table_name,
                                            p.grantee)),
                                            '{}') as grant_privileges
                                   from (select grantee, string_agg(privilege_type, ', ') as privileges
             ...
             cross join lateral (select array_agg(a.attname) as all_columns,
                                        coalesce(array_agg(format(
                                            'alter sequence %%s owned by %%s__tat_new.%%s;',
                                            s.serial_sequence,
                                            tn.table_name,
                                            a.attname)) filter (where s.serial_sequence is not null), '{}') as alter_sequences
                                   from pg_attribute a
             ...
             cross join lateral (select coalesce(array_agg(format(
                                            'create view %%s as %%s; %%s;',
                                            v.oid::regclass::text,
                                            replace(replace(replace(pg_get_viewdef(v.oid),
                                              format('timezone(''Europe/Moscow''::text, %%s.start_time) AS start_time', t.relname), format('%%s.start_time', t.relname)),
                                              format('timezone(''Europe/Moscow''::text, %%s.date_time) AS date_time', t.relname), format('%%s.date_time', t.relname)),
                                              format('timezone(''Europe/Moscow''::text, %%s.hit_time) AS hit_time', t.relname), format('%%s.hit_time', t.relname)),
                                              (select string_agg(format('%%s; %%s;', pg_get_functiondef(pgt.tgfoid), pg_get_triggerdef(pgt.oid)), E';\n')
                                                 from pg_trigger pgt
                                                where pgt.tgrelid = v.oid::regclass))), '{}') as create_views,
                                        coalesce(array_agg(format(
                                            'drop view %%s;',
                                            v.oid::regclass::text)), '{}') as drop_views,
                                        array_agg(v.reltype) as view_type_oids
                                   from pg_class v
                                  where v.relkind = 'v' and
                                        v.oid in (select rw.ev_class
                                                    from pg_depend d
                                                   inner join pg_rewrite rw
                                                           on rw.oid = d.objid
                                                   where d.refobjid = t.oid)) v
             cross join lateral (select coalesce(array_agg(pg_catalog.pg_get_functiondef(f.oid)||';'), '{}') as create_functions,
                                        coalesce(array_agg(format(
                                            'drop function %%s(%%s);',
                                            f.oid::regproc::text,
                                            pg_get_function_identity_arguments(f.oid))), '{}') as drop_functions
                                   from pg_proc f
